...
(function ($, win, undef) {
    ...
    Validform.util = {
        dataType: {
            ...
            "p": /^[0-9]{6}$/,
            "m": /^13[0-9^(/s+)]{9}$|14[0-9^(/s+)]{9}|15[0-9^(/s+)]{9}$|18[0-9^(/s+)]{9}|17[0-9^(/s+)]{9}$/,
            "e": /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
            "url": /^(\w+:\/\/)?\w+(\.\w+)+.*$/
        },
        ...
        cssctl: function (obj, status) {
            switch (status) {
                case 1:
                    ...
                    break;
                case 2:
                    if ((obj.parent().parent().find("input[type!='hidden']").attr("datatype") != undefined && obj.parent().parent().find("input[type!='hidden']").attr("datatype").indexOf("empty") == -1)
                        || obj.parent().parent().find("select").attr("datatype") != undefined && obj.parent().parent().find("select").attr("datatype").indexOf("empty") == -1
                        || obj.parent().parent().find("input[type!='hidden']").attr("datatype") != undefined && obj.parent().parent().find("input[type!='hidden']").val().length > 0 && obj.parent().parent().find("input").attr("datatype").indexOf("empty") > -1)
                        obj.removeClass("Validform_wrong Validform_loading").addClass("Validform_checktip Validform_right");//passed;
                    if (obj.parent().parent().find("input").attr("datatype") != undefined && obj.parent().parent().find("input").val().length == 0 && obj.parent().parent().find("input").attr("datatype").indexOf("empty") > -1) 
                        obj.removeClass("Validform_wrong Validform_loading Validform_right");
            ...
            if (!flag.passed) {
                ...
                if ($(this).attr("ajaxurl") && !bool && $(this).attr("ignore") != "ignore") {
                    ...
                    _this[0].validform_ajax = $.ajax({
                        ...
                        cache: false,
                        url: ajaxurl,
                        data: Validform.util.getQuery.call(win, ajaxurl) + "param=" + inputval + "&name=" + $(this).attr("name") + "&uid=" + $(this).attr("uid"),
                        dataType: "text",
                        success: function (s) {
                            if ($.trim(s) == "y") {
                                ...
                                if (inputobj.attr("disableid") != "" && inputobj.attr("disableid") != undefined) {
                                    $("#" + inputobj.attr("disableid")).prop("disabled", false);
                                }
                                if (inputobj.attr("data_code") != "" && inputobj.attr("data_code") != undefined) {
                                    $("#" + inputobj.attr("data_code")).hide();
                                    $("#" + inputobj.attr("data_code")).find("#code").attr("ajaxurl", "");
                                }
                                if (inputobj.attr("data_codes") != "" && inputobj.attr("data_codes") != undefined) {
                                    $("#" + inputobj.attr("data_codes")).attr("onclick", "settime(this)");
                                }
                            }
                           else if ($.trim(s) == "n") {
                                inputobj[0].validform_valid = "true";
                                ...
                                if (inputobj.attr("data_code") != "" && inputobj.attr("data_code") != undefined) {
                                    $("#" + inputobj.attr("data_code")).show();
                                    $("#" + inputobj.attr("data_code")).find("#code").attr("ajaxurl", "/Admin/Account/VerificationCode");
                                }
                                if (inputobj.attr("disableid") != "" && inputobj.attr("disableid") != undefined) {
                                    $("#" + inputobj.attr("disableid")).prop("disabled", false);
                                }
                                if (inputobj.attr("data_codes") != "" && inputobj.attr("data_codes") != undefined) {
                                    $("#" + inputobj.attr("data_codes")).attr("onclick", "");
                                }
                                ...
                                if (inputobj.attr("disableid") != "" && inputobj.attr("disableid") != undefined) {
                                    $("#" + inputobj.attr("disableid")).attr("disabled", "disabled");
                                }
                                if (inputobj.attr("data_code") != "" && inputobj.attr("data_code") != undefined) {
                                    $("#" + inputobj.attr("data_code")).hide();
                                    $("#" + inputobj.attr("data_code")).find("#code").attr("ajaxurl", "");
