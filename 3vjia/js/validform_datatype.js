/*
    ...
    *6-16：检测是否为6到16位任意字符；
    ...
    那么你绑定datatype="*4-12"就表示4到12位任意字符。如果你自定义了一个datatype="zh2-4"，表示2到4位中文字符，那么datatype="zh2-6"就表示2到6位中文字符。
	...
	daterange：判断日期范围
	idcard：对身份证号码进行严格验证
    idcard：昵称由4-20个数字，字母，符号，汉字组成
    pwd: 密码6-20位字母、数字和符号任意组合
    z:字母
    pro:字母、数字和符号任意组合
    zn:真实姓名，英文长度:4-20 中文长度:2-10
...
(function () {
    if ($.Datatype) {
        $.extend($.Tipmsg.w, {
            ...
            "notvalued": "不能含有特定值！",
            "idcard": "身份证号码不对！",
            "niname": "昵称由2-20个数字，字母，符号，汉字组成",
            "empty": "",
            "pwd": "密码6-20位字母、数字和符号任意组合",
            "z": "字母",
            "username": "帐号由2-20个数字，字母组成",
            "num1":"请填写正整数！",
            "pro": "字母、数字和符号任意组合",
            "zn": "英文长度:4-20 ",
            "cn": "中文长度:2-10",

            "numorAz": "请填写不超过30个数字，英文或组合"
        });
        ...
        $.extend($.Datatype, {
			...
			*/
            ...
            "niname": /^[\u4e00-\u9fa5\@A-Za-z0-9\!\#\$\%\^\&\*\.\~^(/s+)]{2,20}$/,
            "empty": /^\s*$/,
            "pwd": /^(?![\d]+$)(?![a-zA-Z]+$)(?![^\da-zA-Z]+$).{6,20}$/,
            "z": /^[a-zA-Z]*$/,
            "username": /^[0-9a-zA-Z]{2,20}$/,
            "pro": /^[\@A-Za-z0-9\!\#\$\%\^\&\*\.\~^(/s+)]*$/,
            "zn": /^[A-Za-z]{4,20}$/,
            "cn": /^[\u4e00-\u9fa5]{2,10}$/,
            "numorAz":/^[0-9a-zA-Z]*$/
